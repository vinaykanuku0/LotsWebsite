...
const ResponsiveTable = ({ columns, dataTable, filyterKeys, tdWidth, trWidth }) => {
            ...
            <Table className="responsiveTable" ref={firstRowRef}>
              ...
              <Tbody  >
                {currentPosts.map((e, i) => (
                  <Tr key={i} style={{ width: isBroken ? trWidth : "" }}>
                    {columns1?.map((c, j) => (
                      <Td className="p-2 tableresponsive " key={j} style={{ width: isBroken ? tdWidth : "", wordBreak: "break-word" }}>{c?.selector?.(e) || e?.[c?.key]}</Td>
                    ))}
                  </Tr>
                ))}
              </Tbody>
